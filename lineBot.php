...
class Linebot {
	...
	private function httpGet($api,$body){
		$ch = curl_init($api); 
		//curl_setopt($ch, CURLOPT_POST, true); 
		//curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST'); 
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); 
	...
	public function getDisplayName($text){
		$api = "https://api.line.me/v2/bot/profile/".$text;
	...
	public function replyTemplate($text){
		$api = $this->apiReply;
		$webhook = $this->webhookEventObject;
		$replyToken = $webhook->{"events"}[0]->{"replyToken"}; 
		$body["replyToken"] = $replyToken;
	...
	public function postbackdateEvent(){
		$webhook = $this->webhookEventObject;
		$postback = $webhook->{"events"}[0]->{"postback"}->{"params"}->{"datetime"}; 
		return $postback;
	}
